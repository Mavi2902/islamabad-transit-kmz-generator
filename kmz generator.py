...
GTFS_URL = os.getenv("KENTKART_GTFS_URL")
if not GTFS_URL:
    raise RuntimeError(
        "KENTKART_GTFS_URL environment variable is missing. "
...
print("Downloading GTFS feed from:")
print(" ", GTFS_URL)
resp = requests.get(GTFS_URL, timeout=60)
resp.raise_for_status()

gtfs_bytes = io.BytesIO(resp.content)
print("GTFS zip downloaded, size:", len(resp.content), "bytes")
...
print("Trip to route map prepared.")
...
print("GTFS routes + ordered direction-wise stops added.")
...
print("Downloading Metro KMZ from GitHub API (private):")
print(" ", GITHUB_API_METRO_URL)
...
r_metro = requests.get(GITHUB_API_METRO_URL, headers=headers, timeout=60)
...
print("Adding Metro routes from KMZ ...")
...
print("Metro routes added.")

# ============================================================
# SAVE OUTPUT KMZ
# ============================================================

kml_path = os.path.join(BASE_PATH, "Islamabad Transit.kml")
kmz_path = os.path.join(BASE_PATH, "Islamabad Transit.kmz")
